...
# ============================================================
# 六、团队成员（姓名, 职位, 研究兴趣, 照片文件名）
# ============================================================
MEMBERS = [
    ("彭婕",         "特别研究助理", "（待补充）",                "pengjie.jpg"),
    ("韩梦园",       "博士研究生",   "（待补充）",                "hanmengyuan.jpg"),
    ("董伟",         "硕士研究生",   "渤海海峡水动力过程",                "dongwei.jpg"),
    ("乐昆",         "硕士研究生",   "（待补充）",                "lekun.jpg"),
    ("Mannan Aleem", "硕士研究生",   "（待补充）",                "mannan.jpg"),
    ("邱赫",         "硕士研究生",   "风暴潮与淹没范围预报",       "qiuhe.jpg"),
]
...
@media (max-width: 900px) {{
    ...
    .hero .sub {{ font-size: 14px !important; letter-spacing: 0 !important; }}
    .hero .en  {{ font-size: 12px !important; }}
    ...
    .member-name {{ font-size: 15px !important; }}
    .member-role {{ font-size: 12.5px !important; }}
    .member-field {{ font-size: 12.5px !important; }}
    .member-photo-fallback {{ font-size: 40px !important; }}
...
def member_card(name: str, role: str, field: str, photo_filename: str = "") -> str:
    """
    成员卡片：顶部照片（2:3 竖版）+ 下方姓名/职位/研究兴趣。
    如果 R2 上没有该照片，自动显示灰色占位块（人头图标）。
    """
    if photo_filename:
    ...
    return (
        ...
        f'    <div class="member-field"><b>研究兴趣：</b>{field}</div>'
        f'  </div>'
        f'</div>'
    )
...
def page_team():
    ...
    cells = [
        ...
        for name, role, field, photo in MEMBERS
    ]
    grid_row(cells, cols="1fr 1fr 1fr 1fr 1fr 1fr", gap="14px")

    show('<div class="sec-title">团队合影</div>')
    show(f"""
    <div class="img-card" style="max-width:900px; margin:0 auto 16px;">
      <img src="{IMG_TEAM}">
