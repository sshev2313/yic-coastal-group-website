...
def load_local_image(filename: str) -> str:
    ...
    path = os.path.join(here, "figure", filename)
...
BG_URL  = load_local_image("background.png")
MAO_IMG = load_local_image("mao.jpg") or IMG_TEAM
MAP_IMG = load_local_image("map.jpg") or IMG_LAB

BODY_BG = f"url('{BG_URL}')" if BG_URL else "linear-gradient(180deg, #F7FBFE 0%, #EDF5FA 100%)"

# ★ 网页图标：优先用 figure/logo.png，找不到则回退到 🌊
_LOGO_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "figure", "logo.png")
...
.hero h1 {{ font-size: 42px; font-weight: 800; letter-spacing: 3px; margin: 0 0 14px; color: #fff; }}
.hero .sub {{ font-size: 18px; margin: 0; opacity: .95; letter-spacing: 1px; }}
.hero .en  {{ font-size: 14.5px; margin: 10px 0 0; opacity: .72; font-style: italic; }}
...
def grid_row(cells, cols="1fr 1fr", gap="24px"):
    inner = "".join(dedent(c).strip() for c in cells)
    st.markdown(
        f'<div class="grid-row" style="grid-template-columns:{cols};gap:{gap};">{inner}</div>',
...
def img_card(src: str, title: str = "", desc: str = "", min_h: int = 180) -> str:
    body = ""
    if title or desc:
        body = (
    ...
    return (
        ...
        f'<img src="{src}" style="min-height:{min_h}px;">'
        f'{body}'
        f'</div>'
    )
...
def image_carousel(slides, height=440):
    slides_html = ""
    dots_html = ""
...
def page_home():
    ...
    show('<div class="sec-title">研究组掠影</div>')
    slides = [
        (IMG_SHIP, "🌊 海上观测航次",
         "研究组定期开展渤海、黄海近岸海域的多学科综合观测，采集水文、泥沙与生物地球化学样品。"),
        (IMG_TEAM, "🔬 科研一线",
         "将现场观测、遥感反演与三维数值模拟相结合，系统揭示近岸河口动力过程。"),
        (IMG_LAB,  "🧪 实验室分析",
         "样品处理、数据分析与数值模拟一体化研究平台，为高质量成果提供坚实支撑。"),
        (IMG_1,    "🌅 近岸海域",
         "聚焦黄河口及渤海、黄海近岸海域，研究河口羽流扩散与近岸环流结构。"),
        (IMG_2,    "🌊 波浪与潮流",
         "揭示波流共同作用下的泥沙起动、悬扬、输运与沉积动力过程。"),
    ]
    image_carousel(slides, height=440)
...
def page_research():
    ...
    for i, (img, title, desc) in enumerate(items):
        if i % 2 == 0:
            grid_row(
                [img_card(img, "", "", min_h=200), text_card(desc, title)],
                cols="1fr 1.4fr",
            )
        else:
            grid_row(
                [text_card(desc, title), img_card(img, "", "", min_h=200)],
                cols="1.4fr 1fr",
            )
...
def page_team():
    ...
    grid_row([
        f'<div class="img-card">'
        f'<img src="{MAO_IMG}" style="min-height:480px;object-position:top center;">'
        f'</div>',
    ...
    for i in range(0, len(members), 3):
        cells = [
            text_card(f"<b>{role}</b><br>研究兴趣：{field}", f"👤 {name}")
            for name, role, field in members[i:i+3]
        ]
        grid_row(cells, cols="1fr 1fr 1fr", gap="18px")

    show('<div class="sec-title">团队合影</div>')
    show(f"""
    <div class="img-card" style="margin-bottom:16px;height:420px;">
      <img src="{IMG_TEAM}">
      <div class="img-card-body">
        <div class="img-card-title">课题组全体成员（待补充）</div>
      </div>
    </div>
    """)
...
<div style="text-align:center;color:#5A7086;font-size:14px;margin-top:60px;line-height:1.9;
  ...
  © 2025 中国科学院烟台海岸带研究所 · 近岸河口物理海洋研究组<br>
  本站内容仅供学术交流使用
</div>
""")